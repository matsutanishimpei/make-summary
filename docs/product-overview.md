...
| APIへ広すぎるコードを送ると費用とノイズが増える | ローカルでcomment、symbol、import関係、多言語の意味類似度を点数化し、根拠付きで本文送信順を決める | 外部AIへ渡す前に関連範囲を狭め、言語差を越えた候補と選定理由も確認できる |
...
| 関連候補を高解像度で持ち出したい | 安全な候補をすべて対象にし、関連度順で容量上限まで自動梱包 | 人の見落としを避け、追加のAPI/CLI利用なしで容量条件だけ再調整できる |
...
### 7.4 人の確認を残し、手作業の欠落を作らない

AIへ丸投げせず、利用者はプレビュー、警告、未収録理由を確認できます。一方で、関連fileの収録・除外を人へ委ねると重要な依存を落とし得るため、安全な候補はすべて容量最適化へ渡します。人は調査目的と容量を決め、探索範囲と採否は説明可能な自動処理が担います。
...
| 人の役割 | 調査目的と容量上限の入力、成果物・警告・未収録理由の確認 |
...
5. **利用者の手作業で解像度を落とさない**
   プレビュー、警告、未収録理由は示しますが、関連候補のfile単位の採否は安全検証と容量最適化へ委ねます。
...
- プレビュー、キャンセル、自動選定sourceの確認、容量条件を反映するAIなしの再構築
