...
この開発ストーリーを追加するcommitは、上記16commitの後に置かれます。新しいproduct behaviorを追加するcommitではなく、対話とGitに分散していた「なぜ」を時系列へ固定するためのcommitです。
...
これにより、履歴は完成したcodeの説明ではなく、次の判断に使えるproduct memoryになります。
