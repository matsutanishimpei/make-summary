...
仕様が固まるまでの対話は、生成したコンテキストをChatGPTなどのブラウザAIで再利用し、開発エージェントのトークンと費用は仕様確定後の実装・テストへ集中させることを狙っています。
...
2. ルートおよびサブディレクトリの階層`.gitignore`、秘密情報パス、生成物、バイナリ、シンボリックリンクを除外
...
6. 関連ソースを含める・除外する
7. 容量上限を変えた場合は、AIを再実行せずbundleだけ再構築

PC版の「コメント指針」では、ファイル単位の構造化コメントを付ける対象、タグごとの粒度、避ける書き方、変更時の更新チェックをアプリ内で確認できます。
...
1. 「スマホ版」から「プロジェクトを追加」
2. スマホで利用するプロジェクトフォルダを1件または複数件まとめて選択
...
スマホ画面では、登録済みプロジェクトとGemini CLI / Gemini API / Codex CLIを選び、進捗確認、キャンセル、成果物プレビュー、Markdown保存・共有、ZIP保存、容量上限を反映するAIなしのbundle再構築まで行えます。ブラウザの複数ファイル共有が使えない場合はZIP保存へ切り替えてください。
...
| `--embedding <builtin\|off>` | 標準のローカル多言語Embeddingを使うか | `builtin` |
...
`manifest.json` は内部管理用で、添付用ファイル数には含みません。`bundle` 内のMarkdownは必ず指定上限（最大5件）以内です。`01-overview.md` は常に生成し、ツリー、関連ファイルと理由、処理フロー、収録・未収録、警告、不明点、生成日時、GitコミットIDを含みます。要約有効時は、主要コンポーネントの責務、状態とデータの流れ、API、外部依存、修正時の注意点も構造化された調査結果から追加します。同じ意味グループのコードが1成果物へ収まらない場合は、空いている添付枠へ `frontend-2` のように続けて分割します。
...
- Gemini APIへ送る前にも `.gitignore`、秘密情報パス、バイナリ、秘密鍵、代表的なAPIキー・アクセストークン・JWT・コードへ直書きされた認証情報をローカルで検査し、該当ファイルの本文全体を除外します。検出した値そのものはログへ出しません。
...
- ルートから各ファイルの親までの階層`.gitignore`をGit互換の優先順位・否定ルールで評価し、対象ファイルをcore側で拒否します。
- バイナリ、重複、存在しないファイル、プロジェクト外参照、危険なシンボリックリンクをcore側で拒否します。
- コード本文はAI CLIに再生成させず、検証済みの実ファイルを直接読みます。収集直前にも実パス、通常ファイル、ファイル差し替え、`.gitignore`、秘密情報を再検査します。
- 大きな実装ファイルは丸ごと省略せず、収まる最大の連続行範囲へ分けて残します。各断片には元ファイルの行範囲を表示し、overviewでも全文収録と一部収録を区別します。
- AIが返した安全な関連候補は弱い関連もすべて梱包対象にし、強い候補を優先したうえで残容量へ詰めます。file単位の採否は利用者へ委ねず、安全検証と容量上限だけで決定します。
- 出力先はプロジェクト内だけに制限します。
- 成果物は一時ディレクトリへすべて書き終えてから入れ替えます。再生成に失敗した場合は旧成果物を可能な限り復元し、成功時もbundle外の管理外ファイルを保持します。
...
代表的な埋め込みシークレットも検査しますが、独自形式、難読化、分割文字列などを完全には検出できません。成果物を共有する前に内容を確認してください。
...
テストは一時プロジェクト、`InvestigationRunner`、Gemini API HTTP通信のモックを使用し、実際のAI CLIや外部ネットワークを呼びません。coreの正常系、プロバイダー選択、Codex JSONL解析、Gemini APIの構造化出力・補正再試行・認証エラー・利用上限・キャンセル、調査全体のタイムアウトと走査上限、安全なコード索引、代表的な埋め込みシークレット、同一グループの複数成果物分割、最大5件梱包、構造化要約、関連候補の自動収録・容量再構築、原子的な再生成、危険パス、階層`.gitignore`と否定ルール、バイナリ、不正JSON、CLI異常、Windows日本語パス、文字数上限、コード一致、上書き防止を検証します。スマホ連携は未認証拒否、1回限りのQRペアリング、登録プロジェクト制限、実coreを通した生成、MarkdownだけのZIPを検証します。GUIはPC版・スマホ版・共通設定の切り替え、3プロバイダーの選択、共通APIキーの暗号化保存、保存済みキーを使うPC生成、スマホ登録操作を検証します。`test:smoke` は非表示のElectronウィンドウを起動し、3画面、共通設定IPC、モバイルIPC、ビルド済みPWAが実際に読み込まれることを確認します。
...
- Gemini APIへ送るコード本文は最大600,000文字です。ローカル索引は既定で最大25,000ファイル、総読み取り256 MiB、1ファイル8 MiBまで検査します。超過分は省略またはパス一覧のみとなり、大規模リポジトリではCLIより調査精度が下がる場合があります。
- 標準の多言語Embeddingは追加モデル不要の概念・subword hashing方式です。学習済みニューラルモデルほど未知の言い換えを理解できませんが、`EmbeddingProvider`を差し替えられるため、将来ローカルONNXモデル等を追加できます。
